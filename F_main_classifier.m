...
% k: {0,1,2,4,8}
% type: {'baseline','cluster_oracle','cluster_oracle_staged','cluster_offline','cluster_online','cluster_offline_MAB','cluster_online_MAB'}
% window: {1000,5000}
% loc: {'Hyak','Hyakqsub','americano','galao','latte','espresso','mocha','doerlbh','compbio3','c2b2','gcp-gpu'}
%
% e.g.
% F_main_classifier('MNIST', 'stationary', 2, 'cluster_oracle', 1000, 'doerlbh')

function F_main_classifier(dataset, dist, k, type, window, loc)
...
switch dataset
    ...
    case 'CIFAR-100C'
        load test
        prior_x  = double(data(randi(10000,2000,1),:));
        %         prior_y  = double(vec2mat(coarse_labels+1,length(unique(coarse_labels))));
        clearvars data fine_labels coarse_labels filenames batch_label
