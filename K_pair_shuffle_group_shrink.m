...
function [x, y, z, unchosen_x, unchosen_y, unchosen_z, set_size] = K_pair_shuffle_group_shrink(x_all,y_all,z_all,window,v,batch,shrink)

train_index = find(z_all == v);
selected_length = int64(length(train_index)*shrink)
unchosen_index = find(z_all ~= v);
if shrink ~= 0
    if shrink < 1
        unchosen_index = [unchosen_index; train_index(1:selected_length)];
        train_index = train_index(1+selected_length:end);
    else
        add = selected_length - length(train_index);
        train_index = [train_index; unchosen_index(1:add)];
        unchosen_index = unchosen_index(add+1:end);
    end
end
